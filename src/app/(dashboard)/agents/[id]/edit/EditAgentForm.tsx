...
export default function EditAgentForm({
  id, name: n, agencyName: an, phone: p, active: a,
}: { id: string; name: string; agencyName: string; phone: string; active: boolean }) {
  const router = useRouter();
  const [name, setName] = useState(n);
  const [agencyName, setAgencyName] = useState(an);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    setError("");
    const result = await updateAgent(id, { name, agencyName: agencyName || null, phone, active });
    setLoading(false);
    if (!result.success) setError(result.error);
    else { router.push(`/agents/${id}`); router.refresh(); }
  }
