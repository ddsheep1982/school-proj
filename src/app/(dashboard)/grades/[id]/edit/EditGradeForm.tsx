...
import { updateGrade } from "@/actions/grade.actions";
...
export default function EditGradeForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    setError("");
    const result = await updateGrade(id, { name });
    setLoading(false);
    if (!result.success) {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white border border-gray-200 rounded-lg p-6 space-y-4">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 px-3 py-2 bg-gray-50 border border-gray-200 rounded-md">
          {campusName}
        </p>
      </div>
      <div>
